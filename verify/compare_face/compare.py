...
def check_eyes_closed(frame, eye_cascade, face_roi):
    ...
    for (ex, ey, ew, eh) in eyes:
        ...
        eye_status.append(contour_area / (ew * eh) > 0.4)
...
def perform_liveness_check(cap):
    ...
    while (time.time() - start_time) < CONFIG["liveness"]["check_seconds"]:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.putText(frame, status_text, (x, y-40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Blinks: {blink_count}/{CONFIG['liveness']['min_blinks']}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, f"Time left: {int(CONFIG['liveness']['check_seconds'] - (time.time() - start_time))}s", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
def run_full_compare_faces_flow():
    print("\nStarting full compare faces flow...")
    ...
    try:
        while True:
            ...
            if not ret:
                ...
                break
            cv2.putText(frame, "Press 's': Capture Source Face", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, "Press 't': Capture Target Face (Liveness)", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, "Press 'q': Quit", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            face_cascade = cv2.CascadeClassifier(CONFIG["liveness"]["face_cascade"])
